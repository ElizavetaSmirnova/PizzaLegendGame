class TurnCycle {
  ...
  async turn() {
    ...
    if (submission.replacement) {
      ...
      await this.onNewEvent({
        type: "textMessage",
        text: `Иди и возьми их, ${submission.replacement.name}!`
      })
      this.nextTurn();
      return;
    }
    ...
    if (targetDead) {
      await this.onNewEvent({ 
        type: "textMessage", text: `${submission.target.name} проиграл!`
      })
    ...
    if (targetDead) {
      ...
      await this.onNewEvent({
        type: "textMessage",
        text: `${replacement.name} появился!`
      })
    }
  ...
  async init() {
    await this.onNewEvent({
      type: "textMessage",
      text: `${this.battle.enemy.name}  победить вас!`
    })
